...
5.  You can find the following code in the method that is used to save a random generated byte as an encryption key into `UserSecureStore`.

    ```Java
    //If this is single user mode, create and save a key into the user secure store for accessing the offline DB
